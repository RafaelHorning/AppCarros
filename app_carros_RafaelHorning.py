#veiculo = {'modelo': "", 'cor': "", 'Proprietário': ""}
estoque_de_carros = []
limite_estoque = 3
...
def remover():
   #removendo carro do estoque
    if len(estoque_de_carros) > 0:
        ...
        cor_informada = input("Digite a cor do veículo a ser removido do estoque de veículos: ")
        carros_encontrados_por_cor = [carro for carro in estoque_de_carros if cor_informada == str(carro['Cor'])]

        if len(carros_encontrados_por_cor) > 0:
            for carro_encontrado in carros_encontrados_por_cor:
                estoque_de_carros.remove(carro_encontrado)

            print(f"Foram removidos {len(carros_encontrados_por_cor)} veículos com a cor {cor_informada}")
        else:
            print(f"Não foram encontrados veículos com a cor {cor_informada}")
